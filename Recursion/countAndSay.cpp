...
class Solution {
    public:
        string solve(int i, int n, string num) {  //a helper function to solve the recursion
            if(i == n) return num;
            ...
            for(int j = 1; j < num.size(); j++) {
                if(num[j] == num[j-1]) {
                    ...
                    ans += to_string(count) + num[j-1];
                    count = 1;
                }
            }
            // Add the last group
            ans += to_string(count) + num.back();
    
            return solve(i + 1, n, ans);
        }
    
        string countAndSay(int n) {   //main function to call the recursion
            if(n == 1) return "1";
            return solve(1, n, "1");
        }
    };
