...
//USING BINARY SEARCH    (WATCH CODESTORYWITH MIK)
class Solution {
    public:
        int maximumCandies(std::vector<int>& candies, long long k) {
            long long maxC = *max_element(candies.begin(), candies.end());
            long long low = 1, high = maxC, ans = 0;
    
            while (low <= high) {
                long long mid = low + (high - low) / 2; // Prevent overflow
                long long count = 0;
    
                // Count how many children can be served with mid candies each
                for (int candy : candies) {
                    count += candy / mid;
                }
    
                if (count >= k) {  // Key Fix: Check for `>=` instead of `>`
                    ans = mid;  // Store potential answer
                    low = mid + 1; // Try to give more candies per child
                } else {
                    high = mid - 1; // Need to give fewer candies
                }
            }
    
            return ans; // Best found answer
        }
    };